    ...
    public class OrganicQueryHandler : IRequestHandler<OrganicQuery, Organic>
    {
        ...
        private Organic Handler(OrganicQuery request)
        {
            ...
            var organic = new Organic()
            {
                ...
                Carbon = _context.GetSoilLayerTraitData(layers, "Carbon", request.GetItem),
                SoilCNRatio = _context.GetSoilLayerTraitData(layers, "SoilCNRatio", request.GetItem),
                FBiom = _context.GetSoilLayerTraitData(layers, "FBiom", request.GetItem),
                FInert = _context.GetSoilLayerTraitData(layers, "FInert", request.GetItem),
                FOM = _context.GetSoilLayerTraitData(layers, "FOM", request.GetItem)
            };
